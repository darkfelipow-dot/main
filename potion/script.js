...
function initPharmacyCalculator(container) {
  ...
  // ---------- NEW: Custom select helper ----------
  // This replaces the visible UI of a <select> with custom HTML so we can style the options panel
  // (including fully transparent background). The original <select> is kept and updated (hidden).
  function createCustomSelect(selectEl) {
    if (!selectEl) return null;
    // prevent double-init
    if (selectEl.dataset.customized === '1') return selectEl.closest('.custom-select');
    selectEl.dataset.customized = '1';

    const wrapper = document.createElement('div');
    wrapper.className = 'custom-select';
    wrapper.tabIndex = 0;
    // keep the select for form usage but move it inside wrapper and hide visually
    ...
    labelSpan.textContent = selectEl.options[selectEl.selectedIndex]?.textContent || '';
    ...
    Array.from(selectEl.options).forEach((opt, idx) => {
      ...
      o.setAttribute('aria-selected', idx === selectEl.selectedIndex ? 'true' : 'false');
      ...
      o.addEventListener('click', (e) => {
        ...
        optionsWrap.querySelectorAll('.option').forEach(x => x.setAttribute('aria-selected', 'false'));
    ...
    // replace select in DOM with wrapper (select moved inside wrapper)
    ...
    function openCloseToggle(e) {
      ...
      if (isOpen) {
        ...
        const sel = optionsWrap.querySelector('[aria-selected="true"]') || optionsWrap.querySelector('.option');
        sel && sel.scrollIntoView({block: 'nearest'});
      }
    }

    trigger.addEventListener('click', openCloseToggle);

    // keyboard support
    wrapper.addEventListener('keydown', (e) => {
      const open = wrapper.classList.contains('open');
      const focused = optionsWrap.querySelector('[data-focus="1"]');
      if (e.key === 'ArrowDown') {
        e.preventDefault();
        if (!open) {
          ...
          let current = optionsWrap.querySelector('[data-focus="1"]') || optionsWrap.querySelector('[aria-selected="true"]') || optionsWrap.querySelector('.option');
          if (current) {
            let next = current.nextElementSibling;
            while (next && next.classList.contains('disabled')) next = next.nextElementSibling;
            if (next) {
              if (current) current.removeAttribute('data-focus');
              next.setAttribute('data-focus', '1');
        ...
        if (!open) {
          ...
          if (current) {
            ...
            while (prev && prev.classList.contains('disabled')) prev = prev.previousElementSibling;
            if (prev) {
              if (current) current.removeAttribute('data-focus');
              prev.setAttribute('data-focus', '1');
        ...
        if (!open) {
          ...
          // choose focused or selected
          const focusedOpt = optionsWrap.querySelector('[data-focus="1"]') || optionsWrap.querySelector('[aria-selected="true"]');
          if (focusedOpt) {
            const val = focusedOpt.getAttribute('data-value');
            selectEl.value = val;
            selectEl.dispatchEvent(new Event('change', { bubbles: true }));
            labelSpan.textContent = focusedOpt.textContent;
            optionsWrap.querySelectorAll('.option').forEach(x => x.setAttribute('aria-selected', 'false'));
            focusedOpt.setAttribute('aria-selected', 'true');
            wrapper.classList.remove('open');
            trigger.setAttribute('aria-expanded', 'false');
          }
        }
  ...
  // ---- Initialize custom selects inside this form (itemSelect, levelSelect)
  // We run this here so the custom UI is available before other listeners use select.value
  const itemSelect = form.querySelector('#itemSelect');
  const levelSelect = form.querySelector('#levelSelect');
  // createCustomSelect keeps original select element but hides native UI
  if (itemSelect) createCustomSelect(itemSelect);
  if (levelSelect) createCustomSelect(levelSelect);
}